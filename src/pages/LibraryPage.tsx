...
export default function LibraryPage() {
  ...
  useEffect(() => {
    const fetchLibraryData = async () => {
      ...
      try {
        ...
        // --- HATA DÜZELTİLDİ ---
        // API'den dönen nesnenin içindeki 'projects' dizisini alıyoruz.
        // Hata durumunda boş dizi (`|| []`) atayarak kodu daha güvenli hale getiriyoruz.
        setOwnedProjects(ownedResponse.data.projects || []); 
        setFreeProjects(freeResponse.data.projects || []);
  ...
  const displayedProjects = useMemo(() => {
    // ownedProjects veya freeProjects null/undefined ise diye kontrol ekleyelim (daha güvenli)
    const owned = ownedProjects || [];
    const free = freeProjects || [];
    ...
    return combined.filter(project => {
      const categoryMatch = 
        !activeFilters.categories || activeFilters.categories.length === 0 
          ? true
          : project.categories?.some(projectCategory => // project.categories'in varlığını kontrol et
  ...
  const renderContent = () => {
     ...
     if (displayedProjects.length === 0) return <p className="text-center mt-16 text-prestij-text-muted">Kütüphanenizde gösterilecek proje yok veya filtreyle eşleşmiyor.</p>;
      ...
      <div className="p-8 h-full overflow-y-auto">
        <header className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setIsFilterOpen(true)} className="flex items-center space-x-2 px-4 py-2 bg-prestij-bg-button rounded-md hover:bg-prestij-purple/80 transition-colors h-full">
              <Filter size={20} />
            </button>
          </div>
        </header>
